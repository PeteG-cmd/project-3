...
const Modal = ({ handleChange, handleSubmit, closeModal, modalState, comment, bookclub }) => {
  if (!modalState) {
    return null
  }
  return (
...
class EditCommentModal extends React.Component {
  ...
  handleSubmit(event) {
    ...
    const bookClubId = this.props.bookclub._id
    console.log(bookClubId)
    const commentId = this.props.comment._id
    axios.put(`/api/bookclub/${bookClubId}/comment/${commentId}`,
